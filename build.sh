...
# This function prompts the user to press Enter to continue. (Unused in original, but fixed for consistency)
press_enter() {
    cd "$script_path" || exit
    if [ "$auto_continue" = true ]; then
        return
...
info() {
    ...
    # Removed call to show_help "continue"
...
show_help() {
    ...
    # Only prompt if auto_continue is false
    if [ "$auto_continue" = false ]; then
        # The original logic here was slightly flawed with the `input` variable.
        # This simplifies it to either exit or return based on `exit_after`.
        ...
        # If auto_continue is true, just exit if explicitly asked to exit after help.
        if [ "$exit_after" = "exit" ]; then
            exit 0
        fi
        return # Otherwise, just return (no prompt).
    fi
}
...
version=""
edition=""
auto_continue=false # Default to false
...
while (("$#")); do
    ...
    --json | -j | -J)
        ...
        ;;
    --deps | -d | -D)
        run_check_dependencies=true # Set flag to run it after option parsing
        shift
        ;;
    --os)
        os="$2"
    ...
    --version)
        ...
        ;;
    --auto-continue)
        auto_continue=true # Set this to true
        shift
        ;;
    --show | -s | -S)
        show_command=1
    ...
    --debug | -b | -B)
        ...
        ;;
    --help | -h | -H)
        run_show_help=true # Set flag to run it after option parsing
        shift
        ;;
    *)
        # If it's not an option, assume it's config_path
...
if $run_show_help; then
    show_help "exit" # Pass "exit" to ensure it exits if --help was primary action
fi

if $run_check_dependencies; then
    check_dependencies # Execute now that auto_continue is set
fi
...
validate_windows_username() {
    ...
    if [[ ! ${build_username} =~ ^[a-z_]([a-z0-9_-]{0,31}|[a-z0-9_-]{0,30}'\$')$ ]]; then
        echo -e "\n\033[31mBuild username \033[33m$build_username\033[31m is invalid for Windows OS. Please check the build.pkrvars.hcl file."
...
select_os() {
    ...
    # If OS not provided via args and auto-continue is true, error out.
    if [ "$auto_continue" = true ]; then
        print_message error "Error: OS not specified (--os) and auto-continue is enabled. Cannot proceed interactively.\n"
        exit 1
    fi
    ...
    while true; do
        ...
        if [[ $os_input == [qQ] ]]; then
            ...
            info
            clear # Clear after info, so selection prompt comes back cleanly
            print_title
            printf "\nSelect a guest operating system:\n\n"
            for i in "${!os_array[@]}"; do
                printf "$((i + 1)): ${os_array[$i]}\n"
...
select_distribution() {
    ...
    *)
    ...
    # If distribution not provided via args and auto-continue is true, error out.
    if [ "$auto_continue" = true ]; then
        print_message error "Error: Distribution not specified (--dist) and auto-continue is enabled. Cannot proceed interactively.\n"
        exit 1
    fi
...
select_version() {
    ...
    # If version not provided via args and auto-continue is true, error out.
    if [ "$auto_continue" = true ]; then
        print_message error "Error: Version not specified (--version) and auto-continue is enabled. Cannot proceed interactively.\n"
        exit 1
    fi
...
select_edition() {
    ...
    # If edition not provided via args and auto-continue is true, error out.
    if [ "$auto_continue" = true ]; then
        print_message error "Error: Edition not specified (--edition) and auto-continue is enabled. Cannot proceed interactively.\n"
        exit 1
    fi
...
select_build() {
    ...
    if [ ! -d "$INPUT_PATH" ]; then
        ...
        if [ "$auto_continue" = true ]; then
            ...
            return # Ensure return after retry attempt.
        else
            while true; do
    ...
    while true; do
        if [ "$auto_continue" = true ]; then
            ...
            break # Exit this loop and continue with the build.
        fi
    ...
    # ... (rest of the select_build function, which doesn't have prompts and is for variable extraction/command building) ...
...
# Prompt the user to continue or quit. This final loop needs to be guarded.
if [ "$auto_continue" = false ]; then
    while true; do
        action=""
